...
function getPlayer(source) {
	...
	const playerId = source?.id ?? source?.initiator?.id ?? source?.sourceEntity?.id ?? source?.entity?.id;
	if (playerId) return world.getAllPlayers().find(player => player.id === playerId) ?? null;
	return null;
}
...
function showSettings(player) {
	...
	try {
		...
		system.runTimeout(() => {
			...
			}).catch(error => console.error(`[DragonMounts2] Settings form failed: ${error?.message ?? error}`));
		}, 1);
	} catch (error) {
		console.error(`[DragonMounts2] Settings form could not be created: ${error?.message ?? error}`);
	}
}
