...
const DRAGON_ARMOR_LORE = {
  "dragonmounts2:fire_dragon_scale_lore": {
    ...
    maxCd: 45.0,
    getCd: (player) =>
      player.getItemCooldown("dragonmounts2:fire_dragon_scale") / 20,
  },
  ...
  "dragonmounts2:dark_dragon_scale_lore": {
    ...
    maxCd: 30.0,
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("dark_dragon_scale_night_heal");
  ...
  "dragonmounts2:light_dragon_scale_lore": {
    ...
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("light_dragon_scale_day_heal");
      return expiry ? Math.max(0, (expiry - now) / 20) : 0;
    },
  },
  "dragonmounts2:aether_dragon_scale_lore": {
    ...
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("aether_dragon_scale_sprint_speed");
      return expiry ? Math.max(0, (expiry - now) / 20) : 0;
    },
  },
  "dragonmounts2:ice_dragon_scale_lore": {
    ...
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("ice_dragon_scale_defensive_burst");
      return expiry ? Math.max(0, (expiry - now) / 20) : 0;
    },
  },
  "dragonmounts2:forest_dragon_scale_lore": {
    ...
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("forest_dragon_scale_low_health_regen");
      return expiry ? Math.max(0, (expiry - now) / 20) : 0;
    },
  },
  "dragonmounts2:ender_dragon_scale_lore": {
    ...
    getCd: (player, now) => {
      const expiry = cooldowns
        .get(player.id)
        ?.get("ender_dragon_scale_low_health_buff");
      return expiry ? Math.max(0, (expiry - now) / 20) : 0;
    },
  },
