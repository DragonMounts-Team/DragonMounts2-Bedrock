...
world.beforeEvents.worldInitialize.subscribe((event) => {
  event.itemComponentRegistry.registerCustomComponent('dragonmountsplus_whistle_yellow:trigger', {
    onUse: ({ itemStack, source }) => {
      if (!(source instanceof Player)) return;
      if (itemStack.typeId !== 'dragonmountsplus:whistle_yellow') return;
      if (isOnCooldown(source)) return;
      source.runCommandAsync("function item/itemyellow");
